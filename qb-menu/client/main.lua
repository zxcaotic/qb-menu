...
RegisterCommand('test', function()
    local testData = {
        {
            header = "Test Menüsü",
            txt = "Bu bir test menüsüdür",
            icon = "fas fa-vial",
            isMenuHeader = true
        },
        {
            header = "Test Butonu 1",
            txt = "Bu buton test amaçlıdır",
            icon = "fas fa-check",
            params = {
                event = "zxcaotic:test:button1",
                args = {}
            }
        },
        {
            header = "Test Butonu 2",
            txt = "İkinci test butonu",
            icon = "fas fa-star",
            params = {
                event = "zxcaotic:test:button2",
                args = {}
            }
        },
        {
            header = "Devre Dışı Buton",
            txt = "Bu buton devre dışıdır",
            icon = "fas fa-ban",
            disabled = true
        },
        {
            header = "Test Butonu 3",
            txt = "Üçüncü test butonu",
            icon = "fas fa-heart",
            params = {
                event = "zxcaotic:test:button3",
                args = {}
            }
        }
    }
    
    openMenu(testData, true, true)
end, false)

RegisterNetEvent('zxcaotic:test:button1', function()
    QBCore.Functions.Notify('Test butonu 1 tıklandı!', 'success')
end)

RegisterNetEvent('zxcaotic:test:button2', function()
    QBCore.Functions.Notify('Test butonu 2 tıklandı!', 'primary')
end)

RegisterNetEvent('zxcaotic:test:button3', function()
    QBCore.Functions.Notify('Test butonu 3 tıklandı!', 'error')
end)
